...
class TestSummaryReport:
    ...
    def test_summary_sheet_created(self):
        ...
        xlsx_path = Path(self.temp_dir) / "test_summary.xlsx"
        create_template(str(xlsx_path), include_examples=False)
        ...
        for i in range(3):
            ws.cell(row=2+i, column=1, value=i)
            ws.cell(row=2+i, column=2, value="relu")
            ws.cell(row=2+i, column=3, value="1,4,8")
            ws.cell(row=2+i, column=4, value="float32")
            ws.cell(row=2+i, column=7, value="FALSE")
        ...
        # 重新加载检查 summary sheet
        wb = load_workbook(xlsx_path)
        assert "summary" in wb.sheetnames

        ws_summary = wb["summary"]
        # 检查标题
        assert ws_summary.cell(row=1, column=1).value == "比对结果汇总"

    def test_summary_with_mixed_results(self):
        """测试混合结果的汇总"""
        from aidevtools.xlsx.export import _add_summary_sheet
        ...
        # 检查统计数据存在
        values = [cell.value for row in ws.iter_rows(min_row=3, max_row=10, max_col=3) for cell in row]
        assert "PASS" in values
        assert "FAIL" in values
        assert "通过率" in values
    ...
    def test_summary_empty_results(self):
        ...
        ws = wb["summary"]
        # 空结果时总计应为 0
        assert ws.cell(row=1, column=1).value == "比对结果汇总"
